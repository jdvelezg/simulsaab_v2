...
public abstract class SpatialIndexManager implements Cacheable {
	...
	public static synchronized <T> T findNearestObject(Geography<T> geog, Coordinate x, List<Coordinate> closestPoints, Double searchDist) 
		throws NoSuchElementException {
		...
		List<Geometry> close = index.si.query(p.getEnvelope().buffer(searchDist).getEnvelopeInternal());
